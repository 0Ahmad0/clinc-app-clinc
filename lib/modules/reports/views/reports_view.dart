...
class ReportsView extends GetView<ReportsController> {
    ...
    return Scaffold(
      ...
      body: RefreshIndicator(
        ...
        child: CustomScrollView(
          ...
          slivers: [
            // 1. Header الجديد مع الربط القديم
            _buildHeader(theme, cs, isDark),

            // 3. فلتر الفترة الجديد مع الربط القديم
            _buildPeriodTabs(theme, cs),

            // 4. فلتر النوع (مدمج من القديم والجديد)
            _buildTypeFilters(theme, cs),

            // 5. بطاقات KPIs الجديدة
            _buildSummaryCards(theme, cs),

            // 6. الرسم البياني المتقدم (جديد)
            _buildChartSection(theme, cs),

            // 7. أزرار التصدير (جديد)
            _buildExportRow(theme, cs),

            // 8. عنوان القائمة مع العدد
            _buildListHeader(theme, cs),

            // 9. قائمة التقارير (مدمجة)
            _buildReportsList(context, cs),
          ],
        ),
      ),
    );
            ...
            child: Row(
              children: [
                ...
                _PeriodTab(
                  ...
                  onTap: () => controller.changePeriodTab(2),
                ),
              ],
            ),
          );
  ...
  // ===== فلتر النوع (مدمج من القديم والجديد) =====
          ...
          return AnimatedSwitcher(
            ...
            child: period == 0
                ...
                : _MonthlyRevenueChart(
                    key: ValueKey(period == 1 ? 'monthly' : 'yearly'),
                    controller: controller,
                  ),
          );
        }),
  ...
  // ===== قائمة التقارير (مدمجة من القديم والجديد) =====
  Widget _buildReportsList(BuildContext context, ColorScheme cs) {
    return Obx(() {
      final list = List<ReportModel>.unmodifiable(controller.filteredReports);
      final loadingMore = controller.isLoadingMore.value;

      if (list.isEmpty && !loadingMore) {
        return SliverFillRemaining(
          hasScrollBody: false,
          child: _EmptyState(
            cs: cs,
            onGenerate: () => _showFormatSheet(context),
          ),
        );
      }
      ...
      return SliverPadding(
        key: ValueKey(
          '${controller.selectedType.value.name}-${list.length}-$loadingMore',
        ),
        ...
        sliver: SliverList.separated(
          itemBuilder: (_, index) {
            ...
            return ReportListItem(report: list[index]);
          },
  ...
  // ===== عرض نافذة اختيار التنسيق (من الكود القديم) =====
...
class _EmptyState extends StatelessWidget {
  ...
  const _EmptyState({required this.cs, required this.onGenerate});
