...
class AddDoctorView extends GetView<AddDoctorController> {
            ...
            SliverPadding(
              ...
              sliver: SliverList(
                delegate: SliverChildListDelegate([
                  24.verticalSpace,
                  // قسم المعلومات الأساسية (مدمج)
                  _SectionCard(
                    icon: Icons.person_rounded,
                    title: tr(LocaleKeys.add_doctor_basic_info),
                    color: const Color(0xFF3949AB),
                    child: _BasicInfoSection(controller: controller),
                  ),
                  20.verticalSpace,
                  // قسم المعلومات المهنية (مدمج)
                  _SectionCard(
                    icon: Icons.work_rounded,
                    title: tr(LocaleKeys.add_doctor_professional_info),
  ...
  // ===== الهيدر مع Hero (جديد مع ربط قديم) =====
        ...
        background: Container(
          ...
          child: SafeArea(
            child: Column(
              ...
              children: [
                40.verticalSpace,
                // صورة الطبيب (من القديم مع تصميم جديد)
                Obx(
                  () => GestureDetector(
                ...
                4.verticalSpace,
                Text(
                  tr(LocaleKeys.add_doctor_subtitle) ??
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Padding(padding: EdgeInsets.all(18.w), child: child),
        ],
      ),
    );
  }
...
// ===== قسم المعلومات الأساسية (مدمج من القديم والجديد) =====
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        16.verticalSpace,
        // التخصص (من القديم مع تصميم جديد)
        Obx(
          () => _StyledDropdown(
            value: controller.selectedSpecialtyId.value != 0
                ? controller.specializations
                          .firstWhereOrNull(
                            (s) => s.id == controller.selectedSpecialtyId.value,
                          )
                          ?.nameAr ??
                      ''
                : null,
            ...
            color: const Color(0xFF3949AB),
            isLoading: controller.isLoadingSpecializations.value,
            items: controller.specializations.map((s) {
              final isArabic = Get.context?.locale.languageCode == 'ar' ?? true;
              return isArabic ? s.nameAr : s.nameEn;
            }).toList(),
            onChanged: (value) {
              final spec = controller.specializations.firstWhereOrNull(
                (s) => (Get.context?.locale.languageCode == 'ar' ?? true)
                    ? s.nameAr == value
                    : s.nameEn == value,
              );
              if (spec != null) {
                controller.selectedSpecialtyId.value = spec.id;
              }
            },
          ),
        ),
...
// ===== قسم المعلومات المهنية (مدمج من القديم والجديد) =====
...
// ===== قسم الملفات (مدمج من القديم والجديد) =====
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        ...
        // عرض الملفات المرفوعة (من القديم)
        Obx(() {
          if (controller.qualificationFiles.isEmpty)
...
// ===== قسم ساعات العمل (مدمج من القديم والجديد) =====
...
// ===== قسم التوفر (مدمج من القديم والجديد) =====
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isAvailable
                          ? tr('add_doctor.available')
                          : tr('add_doctor.unavailable'),
                      style: theme.textTheme.bodySmall?.copyWith(
                        color: isAvailable
                            ? const Color(0xFF00695C)
                            : cs.onSurfaceVariant,
      ...
      child: Obx(
        () => Container(
          ...
          child: Material(
            ...
            child: InkWell(
              onTap: controller.isLoading.value ? null : controller.saveDoctor,
              borderRadius: BorderRadius.circular(16.r),
              child: Padding(
...
class _StyledDropdown extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return DropdownButtonFormField<String>(
      ...
      decoration: InputDecoration(
        labelText: isLoading ? 'جاري التحميل...' : label,
        prefixIcon: Icon(icon, color: color, size: 18.sp),
        labelStyle: TextStyle(color: cs.onSurfaceVariant, fontSize: 13.sp),
        enabledBorder: OutlineInputBorder(
          borderRadius: BorderRadius.circular(12.r),
  ...
  String? requiredValidator(String? value) {
    if (value == null || value.isEmpty) {
      return tr(LocaleKeys.add_doctor_validations_required);
    }
    return null;
  }
}
